...
public:
    dtype train(const std::vector<std::vector<string> >& sentences, const vector<vector<CAction> >& goldACs) {
        ...
        for (int idx = 0; idx < num; idx++) {
            _pcg->forward(&sentences[idx], 1.0, &goldACs[idx]);
    ...
    void decode(const std::vector<string>& sentence, dtype ratio, vector<string>& result, vector<string>& tags) {
        _pcg->forward(&sentence, ratio);
        predict(result, tags);
    }
...
private:
    ...
    void predict(vector<string>& result, vector<string>& tags) {
        int step = _pcg->outputs.size();
        _pcg->states[step - 1][0].getSegResults(result, tags);
    }
