...
class InstanceWriter : public Writer {
  public:
    ...
    int write(const Instance *pInstance, const vector<string> &curWords, const vector<string> &curTags) {
        if (!m_outf.is_open()) return -1;
    ...
    int write(const Instance *pInstance, const vector<string> &curWords) {
        ...
        if (word_count == 0) return -1;
        for (int i = 0; i < word_count - 1; ++i) {
            m_outf << curWords[i] << "_1" << " ";
        }
        m_outf << curWords[word_count - 1] << "_1"<< std::endl;
        return 0;
    }
};
